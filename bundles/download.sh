if [ ! -f jakartaeetck.zip ]; then
  wget --read-timeout=20 https://download.eclipse.org/ee4j/jakartaee-tck/jakartaee9/milestones/jakartaeetck-9.0.0-M1.zip -O jakartaeetck.zip
fi
if [ ! -f cdi-tck-3.0.0-dist.zip ]; then
  wget https://download.eclipse.org/ee4j/cdi/3.0/cdi-tck-3.0.0-dist.zip -O cdi-tck-3.0.0-dist.zip
fi
...
if [ ! -f latest-glassfish.zip ]; then
	wget https://download.eclipse.org/ee4j/glassfish/glassfish-6.0.0-M1.zip -O latest-glassfish.zip
fi